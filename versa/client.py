...
class Client:
    ...
    def application_id(self) -> Optional[int]:
        ...
        after :func:`~versa.on_connect` is called.
    ...
    def application_flags(self) -> ApplicationFlags:
        """:class:`~versa.ApplicationFlags`: The client's application flags.
    ...
    def _schedule_event(
        ...
        **kwargs: Any,
        ...
        # Schedules the task
        return asyncio.create_task(wrapped, name=f"versa: {event_name}")
    ...
    async def on_error(self, event_method: str, *args: Any, **kwargs: Any) -> None:
        ...
        Check :func:`~versa.on_error` for more details.
        """
    ...
    def allowed_mentions(self) -> Optional[AllowedMentions]:
        """Optional[:class:`~versa.AllowedMentions`]: The allowed mention configuration.
    ...
    def intents(self) -> Intents:
        """:class:`~versa.Intents`: The intents configured for this connection.
    ...
    def users(self) -> List[User]:
        """List[:class:`~versa.User`]: Returns a list of all the users the bot can see."""
    ...
    async def change_presence(
        ...
        *,
        ...
        .. code-block:: python3
            ...
            game = versa.Game("with the API")
            await client.change_presence(status=versa.Status.idle, activity=game)
    ...
    async def fetch_user(self, user_id: int, /) -> User:
        ...
        Retrieves a :class:`~versa.User` based on their ID.
        You do not have to share any guilds with the user to get this information,
        however many operations do require that you do.

        .. note::

            This method is an API call. If you have :attr:`versa.Intents.members` and member cache enabled, consider :meth:`get_user` instead.
    ...
    async def create_dm(self, user: Snowflake) -> DMChannel:
        ...
        Parameters
        ----------
        user: :class:`~versa.abc.Snowflake`
    ...
    def add_view(self, view: View, *, message_id: Optional[int] = None) -> None:
        """Registers a :class:`~versa.ui.View` for persistent listening.
    ...
    def remove_view(self, view: View, message_id: Optional[int] = None) -> None:
        """Removes a :class:`~versa.ui.View` from persistent listening.
    ...
    def add_modal(self, modal: Modal, *, user_id: Optional[int] = None) -> None:
        """Registers a :class:`~versa.ui.Modal` for persistent listening.
    ...
    def remove_modal(self, modal: Modal) -> None:
        """Removes a :class:`~versa.ui.Modal` from persistent listening.
    ...
    async def process_application_commands(self, interaction: Interaction) -> None:
        ...
        if interaction.type is InteractionType.application_command:
            _log.debug("versa.Client: Found an interaction command.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug(
                    "versa.Client: Calling your application command now %s", app_cmd.error_name
                )
                await app_cmd.call_from_interaction(interaction)
            elif self._lazy_load_commands:
                _log.debug(
                    "versa.Client: Interaction command not found, attempting to lazy load."
                )
                # _log.debug(f"versa.Client: %s", interaction.data)
                response_signature = (
                    interaction.data["name"],
                    int(interaction.data["type"]),
                    interaction.guild_id,
                )
                _log.debug("versa.Client: %s", response_signature)
                do_deploy = False
                if app_cmd := self._connection.get_application_command_from_signature(
                    ...
                    _log.debug(
                        "versa.Client: Basic signature matches, checking against raw payload."
                    )
                    if app_cmd.is_interaction_valid(interaction):
                        _log.debug(
                            "versa.Client: Interaction seems to correspond to command %s, associating ID now.",
                            app_cmd.error_name,
                        )
            ...
            _log.debug("versa.Client: Autocomplete interaction received.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug("versa.Client: Autocomplete for command %s received.", app_cmd.name)
    ...
    async def on_guild_available(self, guild: Guild) -> None:
        try:
            if self._rollout_all_guilds or self._connection.get_guild_application_commands(
                ...
                _log.debug(
                    "versa.Client: No locally added commands explicitly registered "
            ...
            _log.warning(
                f"versa.Client: Forbidden error for {guild.name}|{guild.id}, is the commands Oauth scope "
                f"enabled? {e}"
            )
    ...
    def parse_mentions(self, text: str) -> List[User]:
        """Parses user mentions in a string and returns a list of :class:`~versa.User` objects.

        .. note::

            This does not include role or channel mentions. See :meth:`Guild.parse_mentions <versa.Guild.parse_mentions>`
            for :class:`~versa.Member` objects, :meth:`Guild.parse_role_mentions <versa.Guild.parse_role_mentions>`
            for :class:`~versa.Role` objects, and :meth:`Guild.parse_channel_mentions <versa.Guild.parse_channel_mentions>`
            for :class:`~versa.abc.GuildChannel` objects.

        .. note::

            Only cached users will be returned. To get the IDs of all users mentioned, use
            :func:`~versa.utils.parse_raw_mentions` instead.
        ...
        Returns
        -------
        List[:class:`~versa.User`]
            List of :class:`~versa.User` objects that were mentioned in the string.
        """
