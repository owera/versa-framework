...
def show_version():
    ...
    version_info = versa.version_info
    entries.append(
        "- versa v{0.major}.{0.minor}.{0.micro}-{0.releaselevel}".format(version_info)
    )
    if version_info.releaselevel != "final":
        pkg = pkg_resources.get_distribution("versa")
        if pkg:
            entries.append(f"    - versa pkg_resources: v{pkg.version}")
...
_cog_template = '''from versa.ext import commands
import versa
...
def parse_args():
    parser = argparse.ArgumentParser(prog="discord", description="Tools for helping with versa")
