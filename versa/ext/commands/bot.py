...
if TYPE_CHECKING:
    ...
    from versa.activity import BaseActivity
    from versa.enums import Status
    from versa.flags import MemberCacheFlags
    from versa.mentions import AllowedMentions
    from versa.message import Message
    from versa.types.checks import ApplicationCheck, ApplicationHook
...
MISSING: Any = versa.utils.MISSING
...
class MissingMessageContentIntentWarning(UserWarning):
    """Warning category raised when instantiating a :class:`~versa.ext.commands.Bot` with a
    :attr:`~versa.ext.commands.Bot.command_prefix` but without the :attr:`~versa.Intents.message_content`
    intent enabled.

    This warning is not raised when the :attr:`~versa.ext.commands.Bot.command_prefix`
    is set to an empty iterable or :func:`when_mentioned <versa.ext.commands.when_mentioned>`.
...
class BotBase(GroupMixin):
        ...
        *,
        ...
        if (
            ...
            warnings.warn(
                ...
                "See https://docs.versa.dev/en/stable/intents.html#what-happened-to-my-prefix-commands "
    ...
    @versa.utils.copy_doc(versa.Client.close)
    ...
    async def can_run(self, ctx: Context, *, call_once: bool = False) -> bool:
        ...
        return await versa.utils.async_all(f(ctx) for f in data)  # type: ignore

    async def is_owner(self, user: versa.User) -> bool:
        """|coro|
        Checks if a :class:`~versa.User` or :class:`~versa.Member` is the owner of
        this bot.
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise versa.ClientException(f"Cog named {cog_name!r} already loaded")
            self.remove_cog(cog_name)

        cog = cog._inject(self)
        self.__cogs[cog_name] = cog
        # TODO: This blind call to versa.Client is dumb.
        ...
        # this being a "blind call" to versa.Client
        ...
        try:
            if asyncio.iscoroutinefunction(setup):
                try:
                    ...
                    https://docs.versa.dev/en/stable/faq.html#how-do-i-make-my-setup-function-a-coroutine-and-load-it
                    """
                    )
            else:
                setup(self, **extras)
    ...
    async def get_prefix(self, message: Message) -> Union[List[str], str]:
        ...
        if callable(prefix):
            ret = await versa.utils.maybe_coroutine(prefix, self, message)  # type: ignore
        ...
        if isinstance(prefix, str):
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = versa.utils.find(view.skip_string, prefix)
                else:
                    return ctx
...
class Bot(BotBase, versa.Client):
    """Represents a discord bot.

    This class is a subclass of :class:`versa.Client` and as a result
    anything that you can do with a :class:`versa.Client` you can do with
    this bot.
    ...
    command_prefix
        ...
        as its first parameter and :class:`versa.Message` as its second
    ...
    def __init__(
        ...
        *,
        ...
        intents: versa.Intents = versa.Intents.default(),
...
class AutoShardedBot(BotBase, versa.AutoShardedClient):
    """This is similar to :class:`.Bot` except that it is inherited from
    :class:`versa.AutoShardedClient` instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        versa.AutoShardedClient.__init__(
            self,
            max_messages=max_messages,
            connector=connector,
            proxy=proxy,
