...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[versacord.TextChannel, versacord.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`versacord.TextChannel` converter or a :class:`versacord.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`versacord.TextChannel`, and if it fails it tries to convert it to a :class:`versacord.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[versacord.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[versacord.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`versacord.Member` followed by a
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[versacord.Member] = commands.flag(name='member', default=lambda ctx: [])
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[versacord.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: Tuple[versacord.Member, ...]
        reason: str
        days: int = 1
...
When our commands fail to parse we will, by default, receive a noisy error in :data:`~sys.stderr` of our console that tells us
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: versacord.Member):
