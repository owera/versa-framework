...
- ``Channel.is_private``
    ...
    - e.g. ``isinstance(channel, versa.abc.GuildChannel)`` will check if it isn't a private channel.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=versa.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        versa.File('cool.png', 'testing.png'),
        versa.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`versa.utils.find` or :func:`versa.utils.get` via
...
After: ::
    ...
    vc.play(versa.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = versa.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = versa.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = versa.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = versa.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
