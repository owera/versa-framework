...
You can also specify a type annotation such as :class:`vresa.Member`, :class:`vresa.abc.GuildChannel`, or :class:`vresa.Role`
to make the command display a list of those types of objects.

.. code-block:: python3

    @bot.slash_command(guild_ids=[TESTING_GUILD_ID])
    async def hi(interaction: vresa.Interaction, user: vresa.Member):
        """Say hi to a user
